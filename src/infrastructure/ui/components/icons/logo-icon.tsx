...
import { useGlyph, UseGlyphProps } from "../../hooks";

export const LogoIcon = memo<Pick<UseGlyphProps, "size">>(({ size = 7 }) => {
  const { unitWithMeasure } = useGlyph({ size });

  return (
    <svg
      width={unitWithMeasure}
      height={unitWithMeasure}
      viewBox="0 0 20 20"
      fill="none"
